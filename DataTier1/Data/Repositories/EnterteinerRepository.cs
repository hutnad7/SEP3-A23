...
namespace Data.Repositories
{
    public class EnterteinerRepository : BaseRepository<User>, IEnterteinerRepository
    {
        ...
        {
            return await _context.Set<User>().Where(e=> e.Role.Equals(Role.Entertainer))
                .Include(e => e.Bookings).Include(e=> e.Events).ToListAsync();
        }
        public override async Task<ICollection<User>> GetAll(Expression<Func<User, bool>> filter)
        {
            return await _context.Set<User>().Where(e => e.Role.Equals(Role.Entertainer))
                .Include(e => e.Events).Include(e => e.Bookings).Where(filter).ToListAsync();
        }
